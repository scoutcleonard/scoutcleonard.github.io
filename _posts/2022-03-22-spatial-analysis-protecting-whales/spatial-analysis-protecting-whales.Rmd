...
This blog post is based on an assignment from my Master's of Environmental Data Science course [EDS 223: Spatial Analysis for Environmental Data Science](https://jamesfrew.github.io/EDS_223_spatial_analysis/). 

In the assignment, my classmates and I were asked to establish a vessel speed reduction zone around the Caribbean island country of Dominica for the protection of whales from shipping strikes. We were then tasked with assessing the predicted impacts on shipping from such a policy. This is based on a [MESM group project completed in 2020](https://bren.ucsb.edu/projects/whales-vessels-and-fish-economic-valuation-whale-watching-and-marine-spatial-planning). 

### Load Libraries

First, I load the libraries to access all the tools necessary to complete this analysis. I completed this analysis by coding in Python using Jupyter Notebooks. The libraries utilized are shown in the code chunk below: 
...
In the code chunk below, I define a file path for Dominica spatial data so that I have the country's outline shape in order to understand whale and ship traffic around its perimeter:

```python
#filepath to dominica data
...
Then I change the [CRS](https://datacarpentry.org/organization-geospatial/03-crs/) from WGS 84 to Dominica 1945 using the function `to_crs()`. This transforms the CRS to a more accurate geospatial reference system for the region of interest, coastal Dominica:

```python
dominica_2002 = dominica_data.to_crs("EPSG:2002")
```

After transforming the CRS, I plot the outline of Dominica to explore and check that the output is expected:
...
Next, I define the file path for whale sighting spatial data so that I can develop an idea of where whale traffic is before establishing the vessel speed reducation zone, which is intended to protect whales:

```python
#filepath to whale data
sightings_input_folder = r"data"
#name of whale sightings data
sightings_csv = "sightings2005_2018.csv"

sightings_fp = os.path.join(sightings_input_folder, sightings_csv)
```

With that, I file using `read_file()`:
...
In these next steps, I create a grid so that the conservation zone can be established using grid cells with a higher concentration of whale sightings. 

First, I define the grid's boundaries based on the bounds of the whale sightings data:

```python
minx, miny, maxx, maxy = sightings_gdf_2002.total_bounds
```

Then, I define the grid's cell size, in meters:
